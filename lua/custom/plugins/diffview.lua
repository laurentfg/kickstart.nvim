return{
	{
		...
		config = function()
			...
			require("diffview").setup({
				vim.keymap.set('n', '<leader>gv', function() if next(require('diffview.lib').views) == nil then vim.cmd('DiffviewOpen') else vim.cmd('DiffviewClose') end end, { desc = 'Git [V]iew Differences'})
			})
